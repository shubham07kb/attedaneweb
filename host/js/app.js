...
async function presetup(){
    ...
    if(getCookie('isWebAuthnActive')==undefined || getCookie('isWebAuthnActive')==null || getCookie('isWebAuthnActive')==''){
        setCookie('isWebAuthnActive',2,14);
    }
...
function webauthpage(){
    ps('/webauthn');
    gebi('style').innerHTML=logpagecss;
    if(getCookie('isWebAuthnActive')==2 || getCookie('isWebAuthnActive')==1){
        gebi('main').innerHTML=`<div id="telalogin"><div class="container-fundo"><div id="logo-cliente"><h1>CU</h1></div><div class="container-logins"><h2>WebAuthn Activate</h2><br><h4>Fast Login</h4><div class="authenticate">`+fingerprinticon+`</div><div class="login"><button class="botao-login" onClick="ps('/');setCookie('isWebAuthnActive',1,14);location.reload();">Skip</button></div></div></div></div>`;
    } else{
        gebi('main').innerHTML=`<div id="telalogin"><div class="container-fundo"><div id="logo-cliente"><h1>CU</h1></div><div class="container-logins"><div class="login"><input type="text" id="uid" placeholder="UID"><i class="err" id="uiderr"></i><input id="passwd" type="password" placeholder="Passowrd"><i class="err" id="passerr"></i><div class="flex"><p class='ph'>Reset Password</p><p class='ph' id="shasp" onClick="shasp()">Show Password</p></div><button class="botao-login" onClick="loginnow()">Enter</button></div><div class="authenticate">`+fingerprinticon+`</div></div></div></div>`;
    }
}
function loginapply(res){
    ...
    if(res.statusCode==1){
        ...
        if(getCookie('isWebAuthnActive')==2){
            webauthpage();
        }
    }
}
...
async function ps(a,b=sitename){let stateObj={id:"100"};window.history.pushState(stateObj, b, a);route();}
