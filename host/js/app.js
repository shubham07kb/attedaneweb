...
async function applyattencur() {
    gebi('attenapplybg').innerHTML = '<video style="width: 500px; height: 400px; object-fit:cover;" id="vidatt"></video><br><div id="camloadedatt">Loading Camera</div>';;
    Promise.all([
...
async function startattcam() {
    ...
    if ("geolocation" in navigator) {
        navigator.geolocation.getCurrentPosition(
            function (position) {
                g = { stat: 1, lat: position.coords.latitude, lon: position.coords.longitude };
                locrunneddatt(g);
            },
            function (error) {
                gebi("camloadedatt").innerHTML = 'No Location Permission';
